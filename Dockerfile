...
# ==============================================================================
# QLToolsV2 镜像 —— 基于本仓库内嵌源码构建（构建期不访问 GitHub）
#
#   源码目录：./upstream/                    上游源码已随本仓库提供
#   上游项目：https://github.com/nuanxinqing123/QLToolsV2
#   上游提交：见 UPSTREAM.md
#   许可证  ：Apache License 2.0，见 upstream/LICENSE（再分发请保留）
#
#   —— 为什么不需要 Node.js ——
#   上游 web/embed.go 用 //go:embed all:dist 把前端产物 web/dist 编译进
#   二进制，而 web/dist 已随上游源码一并入库（158 个文件）。所以这里只需
#   Go 编译器，既不需要 Node，也不需要单独拷贝静态资源。
#
#   —— 联网需求 ——
#   构建不再克隆上游仓库（源码就在构建上下文里）；
#   仅 Go 模块下载需要网络。若需彻底离线，见 DEPLOY.md「全离线构建」。
# ==============================================================================
...
# 源码全部取自构建上下文内的 upstream/（其无关文件由根目录 .dockerignore 排除）
COPY upstream/ /src/

# 入口包为 ./cmd（cmd/main.go -> internal/app.Start()，对应上游 Makefile 的 run 目标）
# 若 upstream/ 下存在 vendor/ 目录（见 DEPLOY.md「全离线构建」），自动切换为
...
# /app/configs/config.yaml（上游 Viper 读不到配置文件会直接 panic）
ENV TZ=${TZ} \
    APP_MODE=release \
    APP_ADDRESS=0.0.0.0 \
    APP_PORT=1500 \
