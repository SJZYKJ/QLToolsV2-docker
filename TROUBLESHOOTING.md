...
### 5.2 更新模式（`regex_update`）匹配不到，一直退化成新建

更新模式下 `regex_update` 的作用是「**从面板里已有的变量中挑出要被覆盖的那一条**」，判断依据是（`updateExistingVariables`）：

```go
submittedMatch := regex.FindString(newValue)      // 从「客户提交的值」提取
existingMatch  := regex.FindString(e.Value)       // 从「面板里已有的值」提取
if submittedMatch == existingMatch { /* 才允许更新 */ }
```

所以 `regex_update` 必须**同时**能匹配「客户提交的值」和「面板里已有的值」，并且两次提取出来的内容要**完全相等**。常见错法：

- 只用一段精确值当正则（如 `jd_abc123`）→ 换一个账号就匹配不上；
- 正则提取出的片段在两边长度/大小写不同 → 判等失败；
- 结果 `updatedCount == 0`，系统会**静默退化成新建**，于是面板里出现了重复变量（日志里会有「更新模式下未匹配到任何变量，使用新建逻辑」）。

排错时打开日志看这行即可确认：

```bash
docker compose logs -f --tail 200 qltools | grep -E "匹配内容|使用新建逻辑|不匹配正则"
```
