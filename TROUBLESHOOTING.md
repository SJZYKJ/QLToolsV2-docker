...
1. 把同名变量的已有值按 `&`（或换行）拆成若干**字段段**；
2. 用 `regex_update` 分别从「客户提交的值」和「每个字段段」里各提取一次**标识**；
3. 标识相同 → 该段**原位替换**成客户提交的完整值；
4. 标识都不同 → 在末尾**追加**客户提交的值，用 `&` 连接；
5. 合并结果与原有值完全一致 → 不写入（重复提交不会产生重复内容）；
6. 只有**所有面板都没有这个变量名**时，才会新建。

所以 `regex_update` 应该写成「**能唯一识别一段的标识**」，而不是整串。
...
|----------|------|
| `4` | 无命中 → 追加 → `1&2&3&4` |
| `2` | 段 `2` 命中 → 替换 → 仍是 `1&2&3`（不会变成 `1&2&3&2`） |
| `3` | 同上，值已存在 → **跳过写入**，不会重复提交 |
...
- **分隔符**：拆段时 `&` 和换行都算分隔符；拼接时沿用原值已有的风格（原值用换行就仍用换行，否则用 `&`）。
- **判重的是「标识」不是「整串」**：标识相同但内容不同时会**整段替换**——这正是「更新」的意义。想让「内容完全相同才不重复」，就把正则写成能覆盖整段的形式（如 `[^&]+`）。
- **只有第一个命中的段会被替换**：值里出现多个相同标识时，只动最靠前的那一个。

> ⚠️ **行为变更**：早期版本在 `regex_update` 未命中时会**整条覆盖**已有值，或**静默退化成新建**（于是面板里冒出重复变量）。现在统一为「命中替换该段 / 未命中追加」，不会再产生重复变量。
